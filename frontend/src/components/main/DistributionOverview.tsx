      ...
      const fetchDistributionData = async () => {
        try {
          ...
          const response = await axios.post(
            `http://localhost:3001/db/calculate-distribution`,
            { rawScenarioId: rawScenarioId },
          );
          ...
          <TableBody>
            {distributionData.map((row, index) => (
              <TableRow key={index}>
                ...
                <TableCell>
                  {row.relativeA !== null ? row.relativeA.toFixed(2) : "N/A"}%
                </TableCell>
                <TableCell>
                  {row.relativeB !== null ? row.relativeB.toFixed(2) : "N/A"}%
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
