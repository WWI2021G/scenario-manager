...
const InfluencMatrixComponent: React.FC = () => {
  ...
  const [influencingFactors, setInfluencingFactors] = useState<
    InfluencingFactor[]
  >([]);
  const [matrix, setMatrix] = useState<InfluencMatrix>(
    initializeMatrix(influencingFactors),
  );
  ...
  const handleDone = async () => {
    for (let i = 0; i < influencingFactors.length; i++) {
      ...
      await axios
        .post("http://localhost:3001/db/if/as/update", {
          scenarioProject_id: scenarioProject_id,
          name: influencingFactors[i].name,
          description: influencingFactors[i].description,
          activeSum: activeSum,
        ...
        .catch((error) => console.error(error));
      await axios
        .post("http://localhost:3001/db/if/ps/update", {
          scenarioProject_id: scenarioProject_id,
          name: influencingFactors[i].name,
          description: influencingFactors[i].description,
          activeSum: activeSum,
  ...
  const createKeyFactors = async () => {
    ...
    await axios
      .get("http://localhost:3001/db/sp/" + scenarioProject_id)
      .then((response) => {
    ...
    switch (scenarioType) {
      ...
      case "RisikomeidendesSystemszenario":
        ...
        const sortedKeysRMZ: InfluencingFactor[] = mapArrayRMZ
          .map(([key, _value]) => key)
          .slice(0, 20);
      ...
      case "RisikosuchendesSystemszenario":
        ...
        const sortedKeysRSZ: InfluencingFactor[] = mapArrayRSZ
          .map(([key, _value]) => key)
          .slice(0, 20);
  ...
  const postKeyFactors = async (influencingFactors: InfluencingFactor[]) => {
    ...
    for (let i = 0; i < influencingFactors.length; i++) {
      ...
      await axios
        .post("http://localhost:3001/db/ifid", influencingFactors[i])
        .then((response) => {
          influencingFactor_id = response.data.influencingFactor_id;
        })
        .catch((error) => console.error(error));
      await axios
        .post("http://localhost:3001/db/kf/add", {
          scenarioProject_id: scenarioProject_id,
          influencingFactor_id: influencingFactor_id,
        })
        .then((response) => {
          console.log(response);
        })
                ...
                {influencingFactors.map((colFactor) => (
                  ...
                  >
                    {rowFactor.name === colFactor.name ? (
                      ...
                      <Select
                        value={
                          matrix.get(rowFactor.name)?.get(colFactor.name) || 0
                        }
                        onChange={(e) =>
                          handleChange(
                            rowFactor.name,
