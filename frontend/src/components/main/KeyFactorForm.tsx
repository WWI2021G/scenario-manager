...
const KeyFactorForm: React.FC<KeyFactorFormProps> = ({
  onSubmit,
  onCancel,
  initialData,
}) => {
  ...
  const fetchFutureProjections = async () => {
    try {
      ...
      const keyFactor_id = await axios.post("http://localhost:3001/db/kfid", {
        name: initialData.name,
        cur_state: initialData.curState,
      });
      const futureProjections = await axios.get(
        `http://localhost:3001/db/fp/kf/${keyFactor_id.data.keyFactor_id}`,
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await axios
      .post("http://localhost:3001/db/kfid", {
        name: keyFactor_name,
        cur_state: keyFactor_curState,
      })
      .then((response) => {
    ...
    await axios
      .post("http://localhost:3001/db/kf/prop1/update", {
        keyFactor_id,
        prop_one,
      })
      .catch((error) => console.error(error));
    await axios
      .post("http://localhost:3001/db/kf/prop2/update", {
        keyFactor_id,
        prop_two,
      })
      .catch((error) => console.error(error));
    await axios
      .post("http://localhost:3001/db/kf/cstate", {
        name: keyFactor_name,
        cur_state: curState,
      })
      .catch((error) => console.error(error));
  ...
  const onAddFP = async () => {
    ...
    await axios
      ...
      .catch((error) => console.error(error));
    router.push({
      pathname: "/future-projection",
      query: { keyFactor_id: keyFactor_id },
    });
  };
    ...
    >
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between" }}>
        <Button variant="contained" color="primary" type="submit">
          {initialData
            ? "Schlüsselfaktor aktualisieren"
            : "Schlüsselfaktor hinzufügen"}
        </Button>
